...
import { User, Mail, Phone, Save, Edit2, Lock, FileText, BarChart3, BookOpen } from "lucide-react";
...
export default function ExaminerProfile() {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      setFormData({
        ...
        email: userData.authEmail || userData.email || currentUser.email || "",
      ...
      const [examsSnap, questionsSnap] = await Promise.all([
        ...
        getDocs(query(collection(db, "questions"), where("createdBy", "==", currentUser.uid))),
      ]);

      const examIds = examsSnap.docs.map((item) => item.id);
      let pendingEvaluations = 0;

      if (examIds.length > 0) {
        const attemptsSnap = await getDocs(
          query(collection(db, "examAttempts"), where("status", "==", "submitted")),
        );

        pendingEvaluations = attemptsSnap.docs.filter((item) => {
          const attempt = item.data();
          return examIds.includes(attempt.examId) && !attempt.evaluatedAt;
        }).length;
      }

      setStats({
        examsCreated: examsSnap.size,
        questionsCreated: questionsSnap.size,
        pendingEvaluations,
      });
    } catch (error) {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      console.error("Error saving examiner profile:", error);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-8">
      <div className="max-w-5xl mx-auto">
        <div className="mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">Examiner Profile</h1>
          <p className="text-gray-600 mt-2">Manage your account details and security</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <StatCard title="Exams Created" value={stats.examsCreated} icon={FileText} color="blue" />
          <StatCard title="Questions Created" value={stats.questionsCreated} icon={BookOpen} color="green" />
          <StatCard title="Pending Evaluations" value={stats.pendingEvaluations} icon={BarChart3} color="purple" />
        </div>
